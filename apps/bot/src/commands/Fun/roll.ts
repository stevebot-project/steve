...
const DICE_REGEX =
	/(?<count>\d{1,2})?d(?<sides>\d{1,4})(?<explode>!)?(?<keep>kl?(?<keepCount>\d{1,2}))?((?<plus>\+)?(?<minus>-)?(?<mod>\d{1,2})|$)/;
...
@ApplyOptions<SteveCommandOptions>({
	description: "Roll dice like you're on Dimension 20, brought to you by Wilko's mad genius.",
})
@RegisterChatInputCommand((builder, command) =>
	builder
		.setName(command.name)
		.setDescription(command.description)
		.addStringOption((option) => option.setName("dice").setDescription("Your dice roll.").setRequired(true)),
)
export default class extends SteveCommand {
	public override slashRun(interaction: ChatInputCommandInteraction, t: SteveT) {
		const results: DiceResult[] = [];

		const input = interaction.options.getString("dice", true);
		const spec = this.parseDiceRoll(input);
		...
		if (results.length === 1) {
			...
			return interaction.reply(t(LanguageKeys.Commands.Fun.RollEmojiResponse, { emoji, result: result.message }));
		}
		message = LanguageKeys.Commands.Fun.RollResponse;
	...
	private parseDiceRoll(input: string): RollSpec {
		const match = DICE_REGEX.exec(input);
