...
export default class extends SteveCommand {
	...
	public show(msg: KlasaMessage, [key]: [string]) {
		...
		if (path.piece.type === 'Folder') {
			return msg.sendLocale('COMMAND_CONF_USER', [
				key
		...
		return msg.sendLocale('COMMAND_CONF_GET', [path.piece.path, msg.author.settings.resolveString(msg, path.piece)]);
	}
	...
	public async set(msg: KlasaMessage, [key, ...valueToSet]: [string, string[]]) {
		...
		return this.check(msg, key, status) || msg.sendLocale('COMMAND_CONF_UPDATED', [key, msg.author.settings.resolveString(msg, status.updated[0].piece)]);
	}
	...
	public async reset(msg: KlasaMessage, [key]: [string]) {
		...
		return this.check(msg, key, status) || msg.sendLocale('COMMAND_CONF_RESET', [key, msg.author.settings.resolveString(msg, status.updated[0].piece)]);
	}
