...
export default class extends Inhibitor {
	...
	public async run(msg: KlasaMessage, cmd: Command | MusicCommand): Promise<void> {
		if (!(cmd instanceof MusicCommand) || !cmd.music.bitfield || msg.channel.type !== 'text') return;

		/* check if queue is empty */
		if (cmd.music.has(FLAGS.QUEUE_NOT_EMPTY) && !msg.guild.music.canPlay) {
		...
		/* check if playing */
		if (cmd.music.has(FLAGS.VOICE_PLAYING) && !msg.guild.music.playing) {
			if (msg.guild.music.paused) {
		...
		/* check if paused */
		if (cmd.music.has(FLAGS.VOICE_PAUSED) && !msg.guild.music.paused) {
			if (msg.guild.music.playing) {
		...
		if (cmd.music.has(FLAGS.MANAGEABLE) && !msg.guild.music.manageable(msg)) {
